...
describe('pay type = 2 and game type = 1 ', function () {
    ...
    it("account 2 buy op[2] for 6 nas[buyToken]", function(done){
        var args = new Array();
        args.push(gameId);
        args.push(2);
        args.push(1);
        ...
        var tx = new Transaction(ChainID, accounts[2], contractAddress, 
                Unit.nasToBasic(6), ++nonces[2], 1000000, 20000000, contract);
        tx.signTransaction();
        neb.api.sendRawTransaction(tx.toProtoString()).then(function(resp){
            checkTransaction(resp.txhash, function(resp) {
                try{
    ...
    it("sendDeposit", function(done) {
        ...
        var tx = new Transaction(ChainID, accounts[0], contractAddress, 
            Unit.nasToBasic(2), ++nonces[0], 1000000, 20000000, contract);
        tx.signTransaction();
        neb.api.sendRawTransaction(tx.toProtoString()).then(function(resp){
            checkTransaction(resp.txhash, function(resp) {
                try{
    ...
    it("account 3 buy op[2] for 2 nas[buyTicket]", function(done){
        var args = new Array();
        args.push(gameId);
        args.push(2);
        args.push(2);
    ...
    it("account 3 buy op[2] for 1 nas[buyToken]", function(done){
        var args = new Array();
        args.push(gameId);
        args.push(2);
        args.push(2);
        ...
        var tx = new Transaction(ChainID, accounts[3], contractAddress, 
                Unit.nasToBasic(1), ++nonces[3], 1000000, 20000000, contract);
        tx.signTransaction();
        neb.api.sendRawTransaction(tx.toProtoString()).then(function(resp){
            checkTransaction(resp.txhash, function(resp) {
                try{
                ...
                neb.api.getAccountState(accounts[3].getAddressString()).then(function(resp){
                    ...
                    checkNonce(accounts[0], nonces[0], function(){
                        neb.api.getAccountState(accounts[1].getAddressString()).then(function(resp){
                            ...
                            neb.api.getAccountState(accounts[2].getAddressString()).then(function(resp) {
                                console.log(resp);
                                expect((new BigNumber(resp.balance)).sub(new BigNumber(state2.balance)).toString(10)).equal("18000000000000000000");
                                neb.api.getAccountState(accounts[3].getAddressString()).then(function(resp){
                                    console.log(resp);
                                    expect((new BigNumber(resp.balance)).sub(new BigNumber(state3.balance)).toString(10)).equal("2000000000000000000");
                                    done();
                                });
                            });
                        });
